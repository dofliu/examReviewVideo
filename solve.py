...
1.  **務必辨識所有編號的問題**: 無論是「第 N 題」、「Q.N」、「N.」、「Part N」或類似格式,都應視為獨立問題。
2.  **務必辨識所有子問題**: 如「(a)」、「(b)」、「(i)」、「(ii)」等,每個子問題都應有完整的 `problem` 和 `steps`。若 PDF 中題目未明確分出子題,則將所有內容視為單一問題的步驟。
3.  **完整性優先**: 即使某些問題的排版不尋常、跨頁、或包含複雜圖表,仍應盡力解析。若遇到排版混亂的區域,應優先從上下文判斷是否為新的問題。
4.  **確保每個題目都有分數**: 如果 PDF 沒有明確標示分數,請從上下文中推斷一個合理的分數 (例如,其他題目的平均分數)。
5.  **不要遺漏任何題目**: 輸出 JSON 時,請仔細核對圖像中所有可辨識的問題都已包含在 `problems` 列表中。
      ...
      "steps": [
        {
          ...
          "diagram_svg": "對於靜力學/工程題, 在『題目解讀』或『觀念切入』步驟務必提供 SVG 程式碼繪製自由體圖(FBD)或幾何關係圖。其他步驟可選。"
        }
      ]
    }
  ]
...
1. **靜力學必備**: 只要是力學題目, 至少要在前 3 步內提供一個 `diagram_svg`, 畫出受力圖或向量分解。
2. **漸進式繪圖**: 你可以在不同步驟提供不同的 SVG。例如: 第一步畫原始圖, 第二步加上力向量, 第三步加上座標軸。
3. **高品質規範**: 務必定義 `<marker id="arrow">`。向量箭頭必須明顯。

==== ★★★ SVG 繪圖規範 (當需要圖解時使用) ★★★ ====

1. **尺寸與視野**: 使用 `viewBox="0 0 400 300"`, 寬度設為 400, 高度設為 300。背景必須透明。
2. **顏色**:
   - 構件/軸線: `#E8E6D8` (粉筆白)
   - 力向量/重點: `#FFD96B` (粉筆黃)
   - 標註文字: 直接寫在 SVG 的 `<text>` 標籤內, 字體設為 `Arial` 或 `sans-serif`, 填色同對應物件。
3. **物件**:
   - 構件: 使用 `<rect>`, `<line>`, `<circle>`。
   - 向量箭頭: 定義 `<marker id="arrow">` 並在線段末端使用 `marker-end="url(#arrow)"`。
4. **內容**: 繪製自由體圖 (FBD)、力三角形、座標分解圖。圖形應簡潔、標註清晰。
5. **範例**:
   ```xml
   <svg viewBox="0 0 400 300" xmlns="http://www.w3.org/2000/svg">
     ...
     <line x1="50" y1="250" x2="350" y2="250" stroke="#E8E6D8" stroke-width="2" /> <!-- x axis -->
     <line x1="200" y1="250" x2="200" y2="100" stroke="#FFD96B" stroke-width="4" marker-end="url(#arrow)" /> <!-- Force vector -->
     <text x="210" y="120" fill="#FFD96B" font-size="20">F = 100N</text>
   </svg>
   ```
...
5. **不要重複前一步**,每步推進新資訊
6. **公式必須「寫成可以直接念出來的中文」,不要寫算式符號**(display 欄位才用算式符號):
   - 分式 `1/(s+3)`:narration 寫「s 加 3 分之 1」,不要寫「1 除以 s 加 3」
   - 分式 `50/(s²+12s+100)`:narration 寫「s 平方 加 12 s 加 100 分之 50」
   - 三角函數 `sin(30°)`:narration 寫「sine 三十度」,**不要寫 `sin(30°)` 也不要寫 `sine 30 度`**
   - 次方 `s²`:narration 寫「s 平方」,不要寫「s 的 2 次方」也不要寫「s square」
   - 根號 `√(1-ζ²)`:narration 寫「根號 1 減 zeta 平方」
   - 微分 `dy/dx`:narration 寫「dx 分之 dy」
   - 積分 `∫f(x)dx`:narration 寫「f x 對 x 的積分」
7. **接在英文字母(ζ、ωn、sine、K、G、T…)後面的阿拉伯數字,要改成中文字**。原因:TTS 遇到「英文 + 阿拉伯數字」會用英文念法(`30` 被念成 thirty)。改寫範例:
   - `ωn = 10`:narration 寫「omega n 等於 十」而不是「omega n 等於 10」
   - `sine 30°`:narration 寫「sine 三十度」
   - `Kp = 16`:narration 寫「K p 等於 十六」
   - **純計算式裡的阿拉伯數字可以保留**(如 `ζ = 12 / 20 = 0.6`,narration 寫「zeta 等於 12 分之 20 等於 0.6」— 因為前後都是數字或符號,不會被英文化)
   - 判斷準則:看該數字前面一個非空白字是否為英文字母,是就改中文
8. **靜力學/工程專用唸法規則**:
   - **力與分量**: `F1` 唸「F 一」、`F1x` 唸「F 一 的 x 分量」、`FB` 唸「力 F B」。
   - **合力/矩**: `ΣFx = 0` 唸「x 方向的合力等於零」、`ΣMA` 唸「對 A 點的力矩總和」。
   - **單位**: `kN` 唸「千牛頓」、`N-m` 唸「牛頓米」、`MPa` 唸「百萬帕斯卡」、`rad/s` 唸「弧度每秒」。
   - **向量**: `u 軸`, `v 軸` 唸「u 軸」、「v 軸」,確保字母與「軸」之間有空格避免黏字。
   - **角度**: `θ = 30°` 唸「theta 等於 三十度」。
